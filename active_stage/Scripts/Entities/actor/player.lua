...
local C3BalanceMod_ClipOnlyAmmo =
{
	AGLGrenade = true,
	GaussBullet = true,
	HeavyBullet = true,
...
Player = {
	...
	{	
		...
		--
		fileModel = "Objects/Characters/Human/us/nanosuit_v2/nanosuit_v2.cdf",
		shadowFileModel = "Objects/Characters/Human/us/nanosuit_v2/nanosuit_v2_shadow2.cdf", -- shadow model is used for ray traced reflections
		clientFileModel = "Objects/Characters/Human/us/nanosuit_v2/nanosuit_v2.cdf",
...
function Player:SetIsMultiplayer()
	...
	self.Properties.fileModel = "Objects/Characters/Human/us/nanosuit_mp/nanosuit_mp_army.cdf";
	self.Properties.shadowFileModel = "Objects/Characters/Human/us/nanosuit_mp/nanosuit_mp_army.cdf";
	self.Properties.clientFileModel = "Objects/Characters/Human/us/nanosuit_mp/nanosuit_mp_army.cdf";
...
--	AI.ChangeParameter( self.id, AIPARAM_HEATSCALE,1);
	...
	self.c3BalanceModCloakInvisible = nil;
	self:C3BalanceMod_UpdateCloakTargetLoss(true);
...
function Player:C3BalanceMod_IsCloaked()
	if (self.actor and self.actor.GetNanoSuitMode) then
		return self.actor:GetNanoSuitMode() == 2;
	end

	return false;
end

function Player:C3BalanceMod_ForceAITargetLoss()
	if (not AI or not System.GetEntities) then
		return;
	end
	...
	for i, entity in pairs(entities) do
		if (entity and entity.id and entity.id ~= self.id) then
			local aiType = AI.GetTypeOf(entity.id);
			if (aiType and aiType ~= AIOBJECT_NONE and aiType ~= AIOBJECT_PLAYER) then
				local target = AI.GetAttentionTargetEntity(entity.id);
				if (target and target.id == self.id) then
					AI.Signal(SIGNALFILTER_SENDER, 1, "OnLostSightOfTarget", entity.id);
					AI.Signal(SIGNALFILTER_SENDER, 1, "OnTargetNotVisible", entity.id);
					AI.Signal(SIGNALFILTER_SENDER, 1, "OnTargetLost", entity.id);
				end
			end
		end
	end
...
function Player:C3BalanceMod_UpdateCloakTargetLoss(forceScan)
	...
	local cloaked = self:C3BalanceMod_IsCloaked();
	local invisible = 0;
	if (cloaked) then
		invisible = 1;
	end

	if (self.c3BalanceModCloakInvisible ~= invisible) then
		AI.ChangeParameter(self.id, AIPARAM_INVISIBLE, invisible);
		self.c3BalanceModCloakInvisible = invisible;
		if (cloaked) then
			self:C3BalanceMod_ForceAITargetLoss();
		end
	end

	if (cloaked and forceScan) then
		self:C3BalanceMod_ForceAITargetLoss();
	end
end
...
function Player:C3BalanceMod_GetCurrentAmmoName()
	...
	if (item and item.weapon and item.weapon.GetAmmoType) then
		local currentAmmo = item.weapon:GetAmmoType();
		if (currentAmmo) then
			return currentAmmo;
		end
	end
...
function Player:C3BalanceMod_RecordWeaponAmmo(weapon, ammoName)
	if (not weapon or not weapon.GetAmmoCount or not weapon.SetAmmoCount) then
		return;
	end

	local current = weapon:GetAmmoCount();
	if (not current) then
		return;
	end

	local clipSize = nil;
	if (weapon.GetClipSize) then
		clipSize = weapon:GetClipSize();
	end

	if (not self.c3BalanceModWeaponAmmo) then
		self.c3BalanceModWeaponAmmo = {};
	end

	local remembered = self.c3BalanceModWeaponAmmo[ammoName];
	if (not remembered or current > remembered) then
		if (clipSize and clipSize > 0 and current > clipSize) then
			current = clipSize;
		end
		self.c3BalanceModWeaponAmmo[ammoName] = current;
	elseif (current < remembered) then
		local restore = remembered;
		if (clipSize and clipSize > 0 and restore > clipSize) then
			restore = clipSize;
		end
...
function Player:C3BalanceMod_RecordReserveAmmo()
	if (System.IsMultiplayer()) then
		return;
	end
	...
	local ammoName = nil;
	if (item.weapon.GetAmmoType) then
		ammoName = item.weapon:GetAmmoType();
	end
	...
	local capacity = self.inventory:GetAmmoCapacity(ammoName);
	if (not capacity or capacity <= 0) then
		return;
	end

	local current = self.inventory:GetAmmoCount(ammoName);
	if (not current) then
		return;
	end

	if (not self.c3BalanceModReserveAmmo) then
		self.c3BalanceModReserveAmmo = {};
	end

	local remembered = self.c3BalanceModReserveAmmo[ammoName];
	if (not remembered or current > remembered) then
...
function Player.Client:OnShutDown()
	if (AI and self.c3BalanceModCloakInvisible == 1) then
		AI.ChangeParameter(self.id, AIPARAM_INVISIBLE, 0);
	end
