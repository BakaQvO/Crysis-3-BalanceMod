...
ActorShared =
{
  ...
  -- If pain and death are re-enabled they should not be hardcoded in a lua file
	...
	splash_sounds =
	{
		{"Sounds/physics_foleys:foleys/npc:bodyfall_water_deep", SOUND_DEFAULT_3D, 200, 5, 60},
	},
...
--	Bip01 R Hand
	...
	bloodFlowEffect = "",
	bloodSplatWall={
  	"materials/decals/blood/human/blood_splat1",
    "materials/decals/blood/human/blood_splat2",
    "materials/decals/blood/human/blood_splat3",
    "materials/decals/blood/human/blood_splat4",
  	"materials/decals/blood/human/blood_splat5",
    "materials/decals/blood/human/blood_splat6",
    "materials/decals/blood/human/blood_splat7",
    "materials/decals/blood/human/blood_splat8",
  	"materials/decals/blood/human/blood_splat9",
    "materials/decals/blood/human/blood_splat10",
    "materials/decals/blood/human/blood_splat11",
	},
	...
	bloodSplatGround={
		{"materials/decals/blood/human/blood_splatx1", bAnimated = true},
		{"materials/decals/blood/human/blood_splatx2", bAnimated = true},
		{"materials/decals/blood/human/blood_splatx3", bAnimated = true},
	},
...
BasicActorParams =
{
	physicsParams =
	{
		...
		mass_Player = 120,
		mass_Grunt = 180,
		mass_AlienStalker = 250,
		mass_AlienGrunt = 500,
		mass_AlienHeavy = 2000,
		mass_AlienPinger = 15000,
...
function BasicActor:WallBloodSplat(hit, killingHit)
	...
	if ((killingHit and self.deathBloodSplatWall) or self.bloodSplatWall) then
		...
		if ((hits > 0) and splat and ((splat.dist or 0) > 0.5)) then -- Why are we removing hits closer than 0.5 meter?
			...
			-- death blood splat walls are only allowed in vertical surfaces, so
			-- normal angle with the horizontal is not allowed to be greater than 20º
...
function BasicActor.Server:OnHit(hit)
	...
	if(not isMultiplayer) then
		-- ShouldIgnoreHit can be defined on the entity to handle special cases
		-- where the hits should be ignored.
	...
	if (isPlayer and not isMultiplayer and hit.damage and hit.damage > 0 and hit.type ~= "heal") then
		hit.damage = hit.damage * 0.3;
	end
...
--	Log("OnHit >>>>>>>>> "..self:GetName().."   damage: "..hit.damage);
	...
	self.actor:NanoSuitHit(hit.damage, hit.type or "", hit.explosion or false, hit.pos, hit.dir);
...
function BasicActor:KnockedOutByDoor(hit,mass,vel)
	...
	if(force>3)then
		self:PlaySoundEvent("Sounds/physics_world:destructibles:wood_shatter",{x=0,y=0,z=0.6},g_Vectors.v010, SOUND_DEFAULT_3D, 0, SOUND_SEMANTIC_PLAYER_FOLEY);
		local hit = {};
		self:Kill(hit);
	end;
end;
...
function BasicActor:TurnRagdoll()
	--[[ HAX - Is this "hax" needed anymore?
...
function BasicActor:Kill(hit)
	...
	if AI then
		...
		-- Note: We could probably remove ALL these calls, since we don't use them in Crysis 2. /Jonas
...
function BasicActor:DoPainSounds(dead)
	...
	if (self.actor:IsPlayer()) then
		...
		if (sound) then
			local volume = 1.0 - self.actor:GetSuitNoiseSupression();
...
--	In this case, an internal function in CActor is used. If you modify anything
--	with this function, be sure to update CActor::SetActorModelInternal as well.
	...
	elseif (event == "jump_splash") then
		...
		if(not str) then
			...
			if(self.actor:IsLocalClient()) then
				local sound = self:PlaySoundEventEx("sounds/physics_foleys:foleys/npc:bodyfall_water_shallow", SOUND_DEFAULT_3D, 0, volume, g_Vectors.v000, -1, -1, SOUND_SEMANTIC_PLAYER_FOLEY);
			end
		end
...
--[[ not used and also available via AudioSignal -> can be removed from this file
		if(self.actor:IsLocalClient()) then
			local sound = self:PlaySoundEventEx("sounds/physics_foleys:foleys/player:jump_on", SOUND_DEFAULT_3D, 0, volume, g_Vectors.v000, -1, -1, SOUND_SEMANTIC_PLAYER_FOLEY);
		end
--]]
	end
end
...
function BasicActor.Client:OnTimer(timerId,mSec)
	...
	elseif (timerId == HEAVY_LAND_EXPLOSION) then
		g_gameRules:CreateExplosion(self.id, self.id, 100, self:GetPos(), nil, 3.0, nil, 500, nil, nil, 1.0, 3.0, nil, nil, g_gameRules.game:GetHitTypeId("stamp"));
	end
...
function BasicActor:OnStanceChanged(newStance, oldStance)
	...
	if (stanceIsCrouchish) then
		stanceChangeSound = "sounds/physics_foleys:foleys/player:crouch_on";
	else
		stanceChangeSound = "sounds/physics_foleys:foleys/player:crouch_off";
	end
