...
MakeHackable(InteractiveEntity);

local function C3BalanceMod_IsSP()
	return not (System and System.IsMultiplayer and System.IsMultiplayer());
end

local function C3BalanceMod_IsCollectibleTacticalInfo(entity)
	local tacticalInfo = entity and entity.Properties and entity.Properties.TacticalInfo;
	local lookupName = tacticalInfo and tacticalInfo.LookupName;

	return C3BalanceMod_IsSP() and lookupName and string.sub(lookupName, 1, 11) == "Collectible";
end
...
function InteractiveEntity:OnLoad(tbl)
	...
	if (C3BalanceMod_IsCollectibleTacticalInfo(self)) then
		self.bScannable = 1;
		self.Properties.TacticalInfo.bScannable = 1;
	end
...
function InteractiveEntity:Explode()
	...
	if(NumberToBool(self.Properties.Destruction.bExplode))then
  	...
  	if(not EmptyString(explosion.Effect))then
  	  local effectProps = 
  	  { 
  			Scale = explosion.EffectScale,
  		};

  		self.FXSlot = self:LoadParticleEffect( -1, explosion.Effect, effectProps );
...
function InteractiveEntity:CacheResources()
	...
	self:PreLoadParticleEffect( self.Properties.Destruction.Effect );
