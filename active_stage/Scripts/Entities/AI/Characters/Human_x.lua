Human_x =
{
	...
	Properties =
	{
		...
		Damage =
		{
			...
			fileBodyDamageParts = "Libs/BodyDamage/BodyDamageParts_Default.xml",
		...
		esBehaviorSelectionTree = "HumanGrunt",

		fileModel = "objects/characters/human/rebels/rebel_01.cdf",
		bUseFacialFrameRateLimiting = 1,
		equip_EquipmentPack = "CryNetOps_Assault",
		...
		CharacterSounds =
		{
			footstepEffect = "footstep_cell",			-- Footstep mfx library to use
			remoteFootstepEffect = "footstep_cell",		-- Footstep mfx library to use for remote players
			footstepIndGearAudioSignal_Walk = "Cell_Footstep_Gear",   -- This directly plays the specified audiosignal on every footstep
			footstepIndGearAudioSignal_Run = "Cell_Footstep_Gear",	-- This directly plays the specified audiosignal on every footstep
	...
	OnCustomKill = function(entity)
		...
		if (entity.Properties.AI.bPlayDeathSound) then
			entity:PlaySoundEvent('sounds/dialog:dialog:blackops_radio_death', g_Vectors.v000, g_Vectors.v010, SOUND_DEFAULT_3D, 0, SOUND_SEMANTIC_LIVING_ENTITY)
		end
	end,
	...
	OnWaterRippleSeen = function(entity, sender, data)
		...
		if (target and target.actor and target.actor.IsPlayer and target.actor:IsPlayer()) then
			if (target.actor.GetNanoSuitMode and target.actor:GetNanoSuitMode() == 2) then
				return
			end

			local targetCloaked = AI.GetParameter(target.id, AIPARAM_CLOAKED)
			if (targetCloaked == true or targetCloaked == 1) then
				return
			end
		end
...
--	end,
	...
	WatchedMateDie = function(entity)
		...
		elseif (deathBulletType == "typhoonbullet" or deathBulletType == "typhoonshell") then
			AI.PlayCommunication(entity.id, "NoticePlayerUsingTyphoon", "EnemyWeaponComment", 3.0)
		elseif (deathBulletType == "StickyArrow" or deathBulletType == "ElectricArrow" or deathBulletType == "AirBurstArrow" or deathBulletType == "StickyExplosiveArrow") then
			if (AI.GetAlertness(entity.id) >= 2) then
				AI.PlayCommunication(entity.id, "NoticePlayerUsingBow", "EnemyWeaponComment", 3.0)
			end
...
function Human_x:ResetSharedSoundIds()
	...
	self.SharedSoundSignals =
	{
		...
		KilledByKvolt = GameAudio.GetSignal("Kvolt_Finished_FeedbackHit2D" ),
		...
		FeedbackHit2DMike = GameAudio.GetSignal("Mike_FeedbackHit2D" ),
		FeedbackHit2DKvolt = GameAudio.GetSignal("Kvolt_FeedbackHit2D" ),
...
function Human_x:DoPainSounds(dead, hitType, isSilenced)
	...
	if (dead) then
		...
		if(hitType == "mike_burn") then
			AI.PlayCommunication(self.id, "comm_falling_death", "InjuryPain", 0.5);
		elseif (hitType == "silentMelee") then
			return
		else
	...
	if (self.SharedSoundSignals) then
		...
		if (hostile == true) then
			...
			elseif (hit.type == "kvolt") then
				GameAudio.JustPlayEntitySignal(self.SharedSoundSignals.FeedbackHit2DKvolt, self.id);
			elseif (hit.type == "mike_burn") then
				GameAudio.JustPlayEntitySignal(self.SharedSoundSignals.FeedbackHit2DMike, self.id);
