...
function AIBase:GetTargetTypeForComm(targetEntity)
	...
	if (targetEntity) then
		...
		if (string.find(attTargetClassName, "Alien") ~= nil) then
			targetType = "Alien"
		elseif(attTargetClassName == "HumanGrunt") then
			targetType = "Marine"
...
function AIBase:ValidateAttentionTarget(data)
	...
	if (data == nil) then
		...
		if (data.actor and data.actor.IsPlayer and data.actor:IsPlayer()) then
			if (data.actor.GetNanoSuitMode and data.actor:GetNanoSuitMode() == 2) then
				return false
			end

			local targetCloaked = AI.GetParameter(data.id, AIPARAM_CLOAKED);
